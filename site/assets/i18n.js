...
function apply(root=document){if(busy)return;busy=true;try{const r=root.nodeType?root:document;
  const keyed=[];if(r.nodeType===1&&r.matches?.('[data-i18n]'))keyed.push(r);keyed.push(...(r.querySelectorAll?.('[data-i18n]')||[]));keyed.forEach(el=>{const key=el.dataset.i18n;if(dict.messages?.[key]!=null)el.textContent=t(key)});
...
function selectorHosts(){const q=['.topright','#portal > .top .toolbar','#adminApp > .top','.detHead','.head'];const out=[];for(const sel of q){for(const el of document.querySelectorAll(sel)){if(!out.includes(el))out.push(el)}}return out.length?out:[document.body]}
