...
fs.writeFileSync('dist/config.js','window.CC_CONFIG='+JSON.stringify({supabaseUrl:url,publishableKey:key})+';\n');fs.writeFileSync('dist/.nojekyll','');
...
for(const f of fs.readdirSync('dist').filter(f=>f.endsWith('.html'))){const p=path.join('dist',f);let s=fs.readFileSync(p,'utf8');s=s.replace('<head>','<head><meta http-equiv="Content-Security-Policy" content="'+csp+'"><meta name="referrer" content="no-referrer">');fs.writeFileSync(p,s);}
